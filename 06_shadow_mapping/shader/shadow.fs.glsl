...
vec4 calculateSimplePointLight(Light light, Material material, vec3 lightVec, vec3 normalVec, vec3 eyeVec, vec4 textureColor) {
	...
	//Note: You can directly use the shadow related in/uniform variables in this example since we only have 1 light source.
	...
	/*float zShadowMap = texture(u_depthMap, shadowMapTexCoord3D.xy).r;
	...
	for(float dx=-1.0; dx <= 1.0; dx++)
	{
		for(float dy=-1.0; dy <= 1.0; dy++)
		{
			...
			float zShadowMap = texture(u_depthMap, shadowMapTexCoord3D.xy+vec2(dx/u_shadowMapWidth,dy/u_shadowMapHeight)).r;
			if(shadowMapTexCoord3D.z > zShadowMap)
				subShadowCoeff = 0.0;
